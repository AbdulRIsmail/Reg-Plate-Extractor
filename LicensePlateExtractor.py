...
def edge_detection(img):
    ...
    filtered_image = cv2.bilateralFilter(
        gray, pixel_diameter, sigmaColor, sigmaSpace)  # Noise reduction
...
def draw_rect_text_on_img(img, license_plate, polygonal_curves):
    ...
    paint_image = cv2.rectangle(
        img, rectangle_location[0], rectangle_location[1], color, 5)
    paint_image = cv2.putText(img, text=license_plate, org=(
        text_location[0], text_location[1] + offset), fontFace=font, fontScale=1, color=color, thickness=2, lineType=cv2.LINE_AA)
